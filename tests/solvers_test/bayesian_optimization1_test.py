...
from solvers.bayesian_optimization1 import BayesianOptimization
...
def plt_gp(bo, x, y):
    ...
    utility = bo.acq.acqf(x, bo.regressor)
    acq.plot(x, utility, label='Utility Function')
    plt.show()


if __name__=="__main__":
    bo = BayesianOptimization(nlfunc,(np.array([-5]),np.array([10])))
